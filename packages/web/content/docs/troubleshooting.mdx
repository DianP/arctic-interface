...
If copy/paste doesn't work in Arctic on Linux (Ubuntu, Debian, etc.):
...
Restart Arctic after installing.
...
If you're having trouble copying text from Arctic:

```bash
# Use the standard copy shortcut
# Ctrl+C (Linux) or Cmd+C (macOS) after selecting text
