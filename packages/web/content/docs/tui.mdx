...
View comprehensive statistics about your Arctic usage with `/stats`. The dialog has three tabs:

**Overview** - Activity heatmap, favorite model, total tokens, sessions, streaks, and peak coding hour.

![Stats Overview](/tui_stats_overview.png)

**Models** - Visual breakdown of token usage per model with costs.

![Stats Models](/tui_stats_models.png)

**Cost** - Detailed cost summary with token breakdown (input/output/cache) and top models by spending.

![Stats Cost](/tui_stats_cost.png)

**Date Filtering** - Filter stats by time period using the date selector in the top right:
- All time (default)
- Today
- Yesterday
- Last 7 days
...
Use `←`/`→` or `h`/`l` to change the date filter. When filtering by a specific period, the activity heatmap is hidden and streak statistics are omitted since they're not meaningful for filtered views.

Use `Tab` to cycle between tabs, arrow keys to scroll, and `Esc` to close.
