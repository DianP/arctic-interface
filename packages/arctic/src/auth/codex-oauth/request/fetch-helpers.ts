...
type ArcticClient = any;
import { refreshAccessToken } from "../auth/auth";
import { logRequest } from "../logger";
import { getCodexInstructions, getModelFamily } from "../prompts/codex";
...
import type { UserConfig, RequestBody } from "../types";
import {
	PLUGIN_NAME,
	HTTP_STATUS,
	OPENAI_HEADERS,
 ...
 */
export function shouldRefreshToken(auth: Auth.Info): boolean {
	const type = auth.type;
	if (type !== "oauth" && type !== "codex") return false;
	const access = (auth as any).accessToken ?? (auth as any).access;
	const expires = (auth as any).expiresAt ?? (auth as any).expires;
...
export function rewriteUrlForCodex(url: string): string {
	// If the URL already targets the Codex endpoint, leave it as-is
	if (url.includes(URL_PATHS.CODEX_RESPONSES)) {
		return url;
	}

	const index = url.indexOf(URL_PATHS.RESPONSES);
	if (index === -1) {
		return url;
	}
	...
	return (
		...
		url.slice(index + URL_PATHS.RESPONSES.length)
	);
}
...
export async function handleErrorResponse(
	...
	try {
		...
		if (/usage_limit_reached|usage_not_included|rate_limit_exceeded/i.test(code) || response.status === 429) {
            ...
            console.error(`[arctic-codex-plugin] 404 Body:`, raw);
        }

		const enhanced = {
			error: {
				...err,
				message: err.message ?? friendly_message ?? "Request failed.",
				friendly_message,
				rate_limits,
