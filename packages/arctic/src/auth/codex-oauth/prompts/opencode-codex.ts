/**
 * Arctic Codex Prompt Fetcher
 *
 * Fetches and caches the codex.txt system prompt from Arctic's GitHub repository.
...
const CACHE_FILE = join(CACHE_DIR, "arctic-codex.txt")
const CACHE_META_FILE = join(CACHE_DIR, "arctic-codex-meta.json")
...
/**
 * Fetch Arctic's codex.txt prompt with ETag-based caching
 * Uses HTTP conditional requests to efficiently check for updates
 *
 * Rate limit protection: Only checks GitHub if cache is older than 15 minutes
 * @returns The codex.txt content
 */
export async function getArcticCodexPrompt(): Promise<string> {
  ...
  try {
    ...
    throw new Error(`Failed to fetch Arctic codex.txt: ${response.status}`)
  } catch (error) {
    ...
    throw new Error(`Failed to fetch Arctic codex.txt and no cache available: ${error}`)
  }
}

/**
 * Get first N characters of the cached Arctic prompt for verification
