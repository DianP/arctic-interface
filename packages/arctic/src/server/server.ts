...
export namespace Server {
  ...
  export const App = lazy(() =>
    app
      ...
      .use(async (c, next) => {
        const directory = c.req.query("directory") ?? c.req.header("x-arctic-directory") ?? process.cwd()
      ...
      .post(
        "/session/:sessionID/benchmark/start",
        describeRoute({
          summary: "Start benchmark mode",
          description: "Enable benchmark mode on a session and create child sessions for model comparison.",
          operationId: "session.benchmark.start",
          responses: {
            200: {
        ...
        validator(
          ...
          z.object({
            ...
            models: BenchmarkSchema.Model.array().optional(),
        ...
        async (c) => {
          const sessionID = c.req.valid("param").sessionID
          const body = c.req.valid("json")
          const session = await SessionBenchmark.start({
            sessionID,
            count: body.count,
            models: body.models,
      ...
      .post(
        "/session/:sessionID/benchmark/stop",
        describeRoute({
          summary: "Stop benchmark mode",
          description: "Disable benchmark mode for a session and clear benchmark metadata.",
          operationId: "session.benchmark.stop",
          responses: {
            200: {
        ...
        async (c) => {
          const sessionID = c.req.valid("param").sessionID
          const session = await SessionBenchmark.stop({ sessionID })
          return c.json(session)
        },
      )
      .post(
        "/session/:sessionID/benchmark/apply",
        describeRoute({
          summary: "Apply benchmark changes",
          description: "Apply the changes from a benchmark child session to the working tree.",
          operationId: "session.benchmark.apply",
          responses: {
            200: {
              description: "Benchmark changes applied",
              content: {
                "application/json": {
                  schema: resolver(Session.Info),
                },
        ...
        async (c) => {
          const sessionID = c.req.valid("param").sessionID
          const body = c.req.valid("json")
          const session = await SessionBenchmark.apply({
            sessionID,
      ...
      .post(
        "/session/:sessionID/benchmark/undo",
        describeRoute({
          summary: "Undo applied benchmark changes",
          description: "Restore the working tree to the benchmark base snapshot.",
          operationId: "session.benchmark.undo",
          responses: {
            200: {
              description: "Benchmark changes undone",
              content: {
                "application/json": {
                  schema: resolver(Session.Info),
                },
        ...
        async (c) => {
          const sessionID = c.req.valid("param").sessionID
          const body = c.req.valid("json")
          const session = await SessionBenchmark.undo({
            sessionID,
            allowDirty: body.allowDirty,
          })
          return c.json(session)
      ...
      .get(
        ...
        describeRoute({
          ...
          description: "Fetch usage information for configured providers.",
          operationId: "usage.providers",
          responses: {
            200: {
              description: "Provider usage records",
              content: {
                "application/json": {
                  schema: resolver(ProviderUsage.Record.array()),
                },
              },
            },
          },
        ...
        async (c) => {
          ...
          const usage = await ProviderUsage.fetch(undefined, {
          ...
          return streamSSE(c, async (stream) => {
            ...
            const handler = async (event: { directory?: string; payload: any }) => {
              ...
              if (
                ...
                event.payload.properties?.directory === directory
              ) {
                stream.close()
              }
            }
