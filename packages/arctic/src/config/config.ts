...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    if (Flag.ARCTIC_CONFIG) {
      result = mergeConfigWithPlugins(result, await loadFile(Flag.ARCTIC_CONFIG))
      log.debug("loaded custom config", { path: Flag.ARCTIC_CONFIG })
    }
    ...
    if (Flag.ARCTIC_CONFIG_CONTENT) {
      result = mergeConfigWithPlugins(result, JSON.parse(Flag.ARCTIC_CONFIG_CONTENT))
      log.debug("loaded custom config from ARCTIC_CONFIG_CONTENT")
    }

    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        process.env[value.key] = value.token
        const wellknown = (await fetch(`${key}/.well-known/arctic`).then((x) => x.json())) as any
    ...
    if (Flag.ARCTIC_CONFIG_DIR) {
      directories.push(Flag.ARCTIC_CONFIG_DIR)
      log.debug("loading config from ARCTIC_CONFIG_DIR", { path: Flag.ARCTIC_CONFIG_DIR })
    }
  ...
  const selectionCopyDefault = process.platform === "darwin" ? "super+c,meta+c" : "ctrl+shift+c"
  const inputPasteDefault = process.platform === "darwin" ? "super+v,meta+v" : "ctrl+v"
  ...
  export const Keybinds = z
    .object({
      ...
      usage_view: z.string().optional().default("none").describe("View usage"),
      ...
      input_paste: z.string().optional().default(inputPasteDefault).describe("Paste from clipboard"),
      ...
      benchmark_next: z.string().optional().default("ctrl+shift+right").describe("Next benchmark session"),
      benchmark_prev: z.string().optional().default("ctrl+shift+left").describe("Previous benchmark session"),
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "arctic.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "arctic.jsonc"))),
    )
