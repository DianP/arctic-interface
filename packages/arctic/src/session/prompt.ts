...
export namespace SessionPrompt {
  ...
  export const prompt = fn(PromptInput, async (input) => {
    ...
    if (SessionBenchmark.isParent(session) && session.benchmark.enabled) {
      await Session.touch(input.sessionID)
      return benchmarkFanout(session, input)
    }

    if (SessionBenchmark.isChild(session)) {
      const parent = await Session.get(session.benchmark.parentID)
      if (SessionBenchmark.isParent(parent) && parent.benchmark.enabled) {
        return benchmarkChildPrompt(parent, session, input)
      }
    }
  ...
  async function benchmarkFanout(parent: Session.Info, input: PromptInput) {
    ...
    if (parent.benchmark.children.length === 0) {
      const message = await createUserMessage(input)
      if (input.noReply === true) return message
      return loop(input.sessionID)
    }

    const results: MessageV2.WithParts[] = []
    for (const child of parent.benchmark.children) {
      const result = await SessionBenchmark.withWorktreeLock(async () => {
        try {
          const childSession = await Session.get(child.sessionID)
          if (!SessionBenchmark.isChild(childSession)) return undefined
          const worktree = await SessionBenchmark.ensureChildWorktree(parent, childSession)
          const eventDirectory = Instance.directory
          return await Instance.provideContext(
            {
              directory: worktree,
              worktree,
            ...
            async () => {
              ...
              const snapshot = await SessionBenchmark.latestSnapshot(child.sessionID)
              await SessionBenchmark.updateChildSnapshot(child.sessionID, snapshot)
              return message
            },
          )
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error)
          await SessionBenchmark.updateChildSnapshot(child.sessionID, undefined, message)
          return undefined
        }
      })
    ...
    const fallback = parent.benchmark.children[0]
    if (fallback) {
      const messages = await Session.messages({ sessionID: fallback.sessionID, limit: 1 })
      if (messages[0]) return messages[0]
    }
    throw new Error("Benchmark fanout produced no responses.")
  }

  async function benchmarkChildPrompt(
    parent: Session.Info,
    child: Session.Info & { benchmark: BenchmarkSchema.ChildSession },
    input: PromptInput,
  ) {
    return SessionBenchmark.withWorktreeLock(async () => {
      try {
        const parentLatest = await Session.get(parent.id)
        const isApplied =
          SessionBenchmark.isParent(parentLatest) && parentLatest.benchmark.appliedSessionID === child.id
        if (isApplied) {
          const message = await runPrompt({
            ...input,
            sessionID: child.id,
            model: child.benchmark.model,
            messageID: undefined,
          })
          const snapshot = await SessionBenchmark.latestSnapshot(child.id)
          await SessionBenchmark.updateChildSnapshot(child.id, snapshot)
          return message
        }
        if (!SessionBenchmark.isParent(parentLatest)) {
          throw new Error("Parent session is no longer a benchmark parent.")
        }
        const worktree = await SessionBenchmark.ensureChildWorktree(parentLatest, child)
        const eventDirectory = Instance.directory
        return await Instance.provideContext(
          {
            directory: worktree,
        ...
        const message = error instanceof Error ? error.message : String(error)
        await SessionBenchmark.updateChildSnapshot(child.id, undefined, message)
        throw error
      }
    })
  }
  ...
  export const loop = fn(Identifier.schema("session"), async (sessionID) => {
    ...
    while (true) {
      ...
      const result = await processor.process({
        ...
        headers: {
          ...(model.providerID.startsWith("arctic")
            ? {
                "x-arctic-project": Instance.project.id,
                "x-arctic-session": sessionID,
                "x-arctic-request": lastUser.id,
                "x-arctic-client": Flag.ARCTIC_CLIENT,
              }
            : undefined),
          ...model.headers,
        },
  ...
  export async function command(input: CommandInput) {
    ...
    if (input.command === "usage") {
      const modelRef = input.model ? Provider.parseModel(input.model) : await lastModel(input.sessionID)
      return SessionUsage.run({
        sessionID: input.sessionID,
    ...
    if (placeholders.length === 0 && !command.template.includes("$ARGUMENTS") && input.arguments.trim().length > 0) {
