...
export namespace ProviderTransform {
  ...
  function normalizeMessages(msgs: ModelMessage[], model: Provider.Model): ModelMessage[] {
    ...
    // TODO: rm later
    const bugged =
      (model.id === "kimi-k2-thinking" && model.providerID === "arctic") ||
  ...
  export function options(
    ...
    if (
      ...
      (model.providerID === "arctic" && ["kimi-k2-thinking", "glm-4.6"].includes(model.api.id))
    ) {
    ...
    if ((model.api.npm === "@ai-sdk/google" || model.api.npm === "@ai-sdk/google-vertex") && supportsThinking) {
    ...
    if (model.api.id.includes("gpt-5") && !model.api.id.includes("gpt-5-chat")) {
      ...
      if (model.providerID.startsWith("arctic")) {
        result["promptCacheKey"] = sessionID
