...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "arctic.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .arctic directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const arcticDir = path.join(dir, ".arctic")
      await fs.mkdir(arcticDir, { recursive: true })
      const agentDir = path.join(arcticDir, "agent")
...
test("updates config and writes to file", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const writtenConfig = JSON.parse(await Bun.file(path.join(tmp.path, "arctic.json")).text())
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "arctic.json"),
        JSON.stringify({ $schema: "https://usearctic.sh/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "arctic.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .arctic config
      const projectDir = path.join(dir, "project")
      const arcticDir = path.join(projectDir, ".arctic")
      await fs.mkdir(arcticDir, { recursive: true })
      ...
      // Local .arctic config with different plugins
      await Bun.write(
        path.join(arcticDir, "arctic.json"),
        JSON.stringify({
          $schema: "https://usearctic.sh/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .arctic config with some overlapping plugins
      await Bun.write(
        path.join(arcticDir, "arctic.json"),
        JSON.stringify({
          $schema: "https://usearctic.sh/config.json",
